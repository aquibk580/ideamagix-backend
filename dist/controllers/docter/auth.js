...
async function signup(req, res) {
    try {
        const doctotData = doctorSchema.parse(req.body);
        ...
        const token = jwt.sign({ id: doctor.id, email: doctor.email, role: "doctor" }, jwtSecret, { expiresIn: "7d" } // Token expires in 7 days
        );
...
async function signin(req, res) {
    try {
        const doctorData = await doctorSignInSchema.parse(req.body);
        ...
        const token = jwt.sign({ id: doctor.id, email: doctor.email, role: "doctor" }, process.env.JWT_SECRET, { expiresIn: "7d" });
        res.cookie("doctorToken", token, {
            httpOnly: true,
            secure: false,
            sameSite: "lax",
